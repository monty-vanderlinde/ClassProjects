...
public class GPAComparator implements Comparator<Student>
{
     ...
     */
    public GPAComparator(boolean isReversed)
    {
        this.isReversed = isReversed;
    }
    
    /**
     * A method for comparing the GPAs of two student objects.
    ...
    public int compare(Student s1, Student s2)
    {
        ...
        if(s1.getGPA() < s2.getGPA())  //1 if s2 is greater
            returnValue = 1;
        else if(s1.getGPA() > s2.getGPA())  //-1 if s1 is greater
            returnValue = -1;
