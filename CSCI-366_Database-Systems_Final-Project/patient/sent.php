...
<html>
  <head>
    ...
    <?php session_start(); ?>
    <h1 style="font-size:30px;"><u>Sent Messages</u></h1>
    ...
    <?php
      ...
      $stid = oci_parse($conn, "SELECT SentMessageID, RecipientName, TO_CHAR(SentDateTime,'DD-MON-YY HH12:MIAM') FROM SentMessage WHERE SentMessage.PatientID = ".$_SESSION["patient_id"]);
      $r = oci_execute($stid);
    ...
    <?php
      ...
      if($messageID != 0){
        $conn = oci_connect('monvande', 'Islandk14', '(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(Host=db2.ndsu.edu)(Port=1521)))(CONNECT_DATA=(SID=cs)))');
        $stid = oci_parse($conn, "SELECT MessageText FROM SentMessage WHERE SentMessage.SentMessageID = ".$messageID);
        $r = oci_execute($stid);
